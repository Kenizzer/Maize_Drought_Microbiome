...
Get_taxa_importance <- function(RF_MODEL = NULL, FOLD_MODEL = NULL, TOP_N = 100){
  if (!is.null(RF_MODEL)){
    ...
    for (i in my_vect){
      taxa_df[i, ] <- c(drt.fungi.late.clr@tax_table[i,])
    }
...
RF_list_soil_inoculum <- plyr::join(data.frame('ASV' = rownames(drt.fungi.late.clr@tax_table)), RF_list_soil_inoculum) # Fix to adjust order of dataframe to match phyloseq_taxtable
...
RF_list_genotype <- plyr::join(data.frame('ASV' = rownames(drt.fungi.late.clr@tax_table)), RF_list_genotype) # Fix to adjust order of dataframe to match phyloseq_taxtable
RF_list_genotype$ASV_numb <- paste("ASV", rownames(RF_list_genotype))

# Subplots for each main effect
a <- ggplot(RF_list_treatment[RF_list_treatment$mean > 0.001, ], aes(x= mean, y = reorder(ASV_numb, mean), fill = Class)) +
  geom_col() +
